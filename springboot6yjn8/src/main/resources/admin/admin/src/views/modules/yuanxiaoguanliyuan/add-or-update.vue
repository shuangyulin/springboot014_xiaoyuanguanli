<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="院校账号" prop="yuanxiaozhanghao">
          <el-input v-model="ruleForm.yuanxiaozhanghao" 
              placeholder="院校账号" clearable  :readonly="ro.yuanxiaozhanghao"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
            ...
            ruleForm: {
                	        yuanxiaozhanghao: '',
	                        	        ...
	                        	        beizhu: '',
                                                              ...
                                                              rules: {
                  yuanxiaozhanghao: [
                            { required: true, message: '院校账号不能为空', trigger: 'blur' },
                                    	                                                              ],
                  mima: [
  ...
  methods: {
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                                                ...
                                                                // 多级联动参数
                                                                                  info(id) {
      this.$http({
        url: `yuanxiaoguanliyuan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `yuanxiaoguanliyuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yuanxiaoguanliyuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
